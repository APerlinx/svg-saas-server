...
jest.mock('../../../middleware/monthlyGenerationQuota', () => ({
    monthlyGenerationQuota: () => (req, res, next) => next(),
    incrementGenerationQuota: jest.fn(),
}));
jest.mock('../../../middleware/rateLimiter', () => ({
    createPlanRateLimiter: () => (req, res, next) => next(),
}));
...
describe('POST /v1/svg/generate - Generate SVG', () => {
    const { createGenerationJob, enqueueGenerationJob, } = require('../../../services/svgGenerationService');
    const { incrementGenerationQuota, } = require('../../../middleware/monthlyGenerationQuota');
    beforeEach(() => {
        jest.clearAllMocks();
    ...
    it('should return 400 if request body is invalid', async () => {
        const res = await (0, supertest_1.default)(app).post('/v1/svg/generate').send('invalid');
        expect(res.status).toBe(400);
        expect(res.body.error).toBe('Invalid request body');
    });
    it('should return 400 if prompt is missing', async () => {
        const { ValidationError, } = require('../../../services/svgGenerationService');
        createGenerationJob.mockRejectedValue(new ValidationError('Prompt is required'));
    ...
    it('should pass API key ID to job creation', async () => {
        ...
        prisma.generationJob.update.mockResolvedValue({});
        enqueueGenerationJob.mockResolvedValue(undefined);
        await (0, supertest_1.default)(app).post('/v1/svg/generate').send({ prompt: 'test' });
