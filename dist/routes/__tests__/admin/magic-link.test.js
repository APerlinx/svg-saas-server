...
describe('Admin Magic Link Authentication', () => {
    ...
    describe('POST /api/admin/request-access', () => {
        it('should send magic link to admin email', async () => {
            ;
            emailService.sendAdminMagicLink.mockResolvedValue(undefined);
            ...
            expect(emailService.sendAdminMagicLink).toHaveBeenCalledWith('admin@chatsvg.dev', expect.any(String));
        });
        it('should not reveal if email is incorrect', async () => {
            ...
            expect(response.status).toBe(200);
            expect(response.body.message).toBe('If this email is registered as admin, a magic link was sent');
        ...
        it('should handle email service errors', async () => {
            ;
            emailService.sendAdminMagicLink.mockRejectedValue(new Error('Email service down'));
            const response = await (0, supertest_1.default)(app)
                .post('/api/admin/request-access')
                .send({ email: 'admin@chatsvg.dev' });
    ...
    describe('GET /api/admin/auth', () => {
        it('should set admin session cookie and redirect with valid token', async () => {
            ...
            const response = await (0, supertest_1.default)(app).get(`/api/admin/auth?token=${token}`);
        ...
        it('should require token parameter', async () => {
            const response = await (0, supertest_1.default)(app).get('/api/admin/auth');
            expect(response.status).toBe(400);
    ...
    describe('GET /api/admin/metrics', () => {
        it('should require admin authentication', async () => {
            const response = await (0, supertest_1.default)(app).get('/api/admin/metrics');
            expect(response.status).toBe(401);
    ...
    describe('POST /api/admin/logout', () => {
        it('should clear admin session cookie', async () => {
            const response = await (0, supertest_1.default)(app).post('/api/admin/logout');
            expect(response.status).toBe(200);
            expect(response.body.message).toBe('Logged out successfully');
            const cookies = response.headers['set-cookie'];
            expect(cookies).toBeDefined();
            expect(cookies.some((c) => c.includes('admin_session=') && c.includes('Expires'))).toBe(true);
        });
    });
});
